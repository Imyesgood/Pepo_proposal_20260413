...
from output.tables import build_ytm_table, build_rolling_matrix, AssetRow
...
PALETTES = {
    "포인트 (그린 계열)": ["#1B5E20","#354F52","#606C38","#2C7873","#4A5568","#6B4226","#52796F","#37474F"],
    "차분 (뮤트 뉴트럴)": ["#5C6B73","#7D8E8E","#8C7B6B","#6B7C6B","#707070","#8A7F72","#4F6367","#7A6C5D"],
    "모노크롬":           ["#1A1A1A","#3D3D3D","#5C5C5C","#787878","#949494","#ABABAB","#C2C2C2","#D8D8D8"],
}
...
def render(bond_groups=None, rc_scenarios=None):
    ...
    weight_repo  = c4.number_input("REPO NAV비중(%)", value=200, step=10, key="pv_repo") / 100

    fp_start = date(2026, 8, 25)
    fp_end   = date(2027, 2, 25)

    # ── 편입자산 구성 — 수익률 계산 탭의 그룹에서 선택 ────────────────────────
    ...
    # 그룹 이름 목록
    group_names = [g["name"] for g in bond_groups]

    # 기본 포트폴리오 / 레버리지 포트폴리오 구분 설정
    st.caption("각 그룹의 역할과 NAV 대비 투자비중을 설정하세요.")
    asset_rows_custom = []
    c_hdr = st.columns([2,2,1,1])
    c_hdr[0].markdown("**그룹**"); c_hdr[1].markdown("**구분**")
    c_hdr[2].markdown("**NAV비중(%)**"); c_hdr[3].markdown("**포함**")
    ...
    for i, grp in enumerate(bond_groups):
        ...
        cg = st.columns([2,2,1,1])
        grp_role = cg[1].selectbox("", ["기본 포트폴리오","레버리지 채권"],
                                    key=f"pv_role_{i}", label_visibility="collapsed")
        grp_wt   = cg[2].number_input("", value=int(grp["weight_nav"]*100), step=10,
                                       key=f"pv_wt_{i}", label_visibility="collapsed") / 100
        grp_inc  = cg[3].checkbox("", value=True, key=f"pv_inc_{i}", label_visibility="collapsed")
        cg[0].markdown(f"**{grp['name']}** `{ytm_avg:.3f}%`")

        if grp_inc:
            selected_groups.append({
                "grp": grp, "role": grp_role, "weight": grp_wt, "ytm": ytm_avg
            })

    # REPO 행 자동 추가
    asset_rows_list = []
    # 기본 포트폴리오
    for sg in [s for s in selected_groups if s["role"]=="기본 포트폴리오"]:
        asset_rows_list.append(AssetRow("기본\n포트폴리오", sg["grp"]["name"],
                                        sg["ytm"], ml(fund_mat), sg["weight"]))
    # REPO
    asset_rows_list.append(AssetRow("레버리지\n포트폴리오", "REPO 매도",
                                    "기준금리+5bp", "1일", -weight_repo, is_repo=True))
    # 레버리지 채권
    for sg in [s for s in selected_groups if s["role"]=="레버리지 채권"]:
        asset_rows_list.append(AssetRow("레버리지\n포트폴리오", sg["grp"]["name"],
                                        sg["ytm"], ml(fund_mat), sg["weight"]))
    # 현금
    ...
    # Tab4에서 시나리오 가져왔으면 그대로, 없으면 여기서 직접 설정
    if rc_scenarios:
        st.caption("수익률 계산 탭의 시나리오를 사용합니다.")
        scenarios = {s["name"]: s["changes"] for s in rc_scenarios}
    else:
        st.caption("수익률 계산 탭에서 시나리오를 설정하거나 아래에서 직접 입력하세요.")
        bok_in = [d for d in BOK_DATES if fp_start<d<=fp_end]
        n_sc   = int(st.number_input("시나리오 수",1,6,2,key="pv_ns"))
        name_cols = st.columns(n_sc)
        sc_names  = [name_cols[i].text_input(f"시나리오{i+1}",
                     value=["동결",f"{bok_in[0].month}월 인하","2회 인하","S4","S5","S6"][i],
                     key=f"pv_sn{i}") for i in range(n_sc)]
        sc_changes = [{} for _ in range(n_sc)]
        for j,bok_d in enumerate(bok_in):
            row = st.columns([1]+[2]*n_sc)
            row[0].markdown(f"`{bok_d.strftime('%m/%d')}`")
            for i in range(n_sc):
                default_bp = (-25 if (i==1 and j==0) or (i==2 and j<2) else 0)
                bp = row[i+1].selectbox("",BP_OPTIONS,format_func=lambda x:BP_LABELS[x],
                                         index=BP_OPTIONS.index(default_bp),
                                         key=f"pv_bp_{i}_{j}",label_visibility="collapsed")
                if bp!=0: sc_changes[i][bok_d] = bp
        scenarios = {sc_names[i]:{d:bp/10000 for d,bp in sc_changes[i].items()} for i in range(n_sc)}

    sc_cols = list(scenarios.keys())
    # repo 미리보기
    rp = [f"**{sc}** {(calc_weighted_avg_rate(fp_start,fp_end,cur_rate,chg)+REPO_SPREAD)*100:.3f}%"
          for sc,chg in scenarios.items()]
    st.caption("레포금리:  " + "  |  ".join(rp))

    # 팔레트
    pal_name = st.selectbox("차트 컬러", list(PALETTES.keys()), key="pv_pal")
    ...
    st.markdown(f"<span style='color:{COLORS['deep_green']};font-size:1.05rem;font-weight:700'>"
                ...
                f"연환산, {ref_date} 기준</span>", unsafe_allow_html=True)

    t1 = build_ytm_table(asset_rows_list, scenarios, fp_start, fp_end, cur_rate)
    ...
    st.dataframe(
        t1.style.format(lambda x:f"{x:.3f}%" if isinstance(x,float) else x, subset=sc_cols)
    ...
    st.divider()
    st.markdown(f"<span style='color:{COLORS['deep_green']};font-size:1.05rem;font-weight:700'>"
                f"■ 롤링 효과 반영 및 금리변동 시나리오별 예상수익률</span>", unsafe_allow_html=True)

    hold_years = hold_m / 12
    duration   = fund_mat - hold_years
    tgt_date   = pd.Timestamp(all_dates[0])
    slope      = get_slope(sector_data,"공사채(AAA)",tgt_date,max(fund_mat-hold_years,0.25),fund_mat)
    rolldown   = hold_years * slope
    st.caption(f"rolldown={rolldown:.4f}%  |  잔존듀레이션={duration:.2f}Y")

    dy_opts   = st.multiselect("시중금리 변동폭", [-75,-50,-25,0,25,50,75], default=[-25,0,25],
    ...
    st.dataframe(
        t2.style.format(lambda x:f"{x:.3f}%" if isinstance(x,float) else x, subset=sc_cols)
